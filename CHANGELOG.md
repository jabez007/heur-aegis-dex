...
- **Speed Boost and Protean are scored.** An audit of every ability the 208 legal species carry found 182 distinct abilities, of which 30 were modelled and applied. Most of the remainder genuinely do nothing this tool can measure — Keen Eye, Gluttony, Frisk — but four Pokémon were carrying Speed Boost and two Protean with no credit at all.
  ...
  **Mold Breaker is recorded and rejected on measurement rather than principle.** It needs no moveset, and unlike Marvel Scale it does not need the opponent to blunder — only to have brought a Pokémon whose ability changes how a move lands. The pool this tool already scores is a fair stand-in for the opponent, so the expected value is computable. Measured across all 208 legal species: 20.7% carry a selected ability it would turn off, worth a mean 0.0224 of member quality, for an expected **0.0046** — about a seventh of Sturdy, the smallest applied entry. The abilities it meets most often are the cheap ones (Sturdy seven times, Flash Fire six) while the ones worth removing are singletons: one Dragonite at 0.0900, one Furfrou at 0.0816. 79% of the time it does nothing at all, and an average that small at that spread is a coin-flip dressed as a rating.

  The category carries a warning of its own in the docstring. Crediting Mold Breaker, Unnerve or Pressure turns the file into a matchup model, and a matchup model does not stop at one ability — Defiant answers Intimidate, Good as Gold answers status. That should be a decision taken on purpose, not one arrived at by adding a seventh reasonable-looking multiplier.
