...
describe('type threat weights', () => {
  ...
  it('counts a pool member’s own types in full', () => {
    const shares = measureTypeThreat(
      [stabOnly('unknown-a', ['fire']), stabOnly('unknown-b', ['water'])],
      TYPES
    );
  ...
  it('splits the moveslots left after STAB across the types a Pokemon can reach', () => {
    ...
    const shares = measureTypeThreat([garchomp], TYPES);
  ...
  it('never lets a Pokemon run more coverage than it can reach', () => {
    ...
    const shares = measureTypeThreat([{ name: 'garchomp', types: ['ground'] }], TYPES);

    expect(Math.max(...Object.values(shares))).toBe(1);
  });

  it('normalizes so the most available attacking type weighs exactly 1', () => {
    const weights = getTypeThreatWeights(getThreatPool(catalog, { baseScore: 18 }), TYPES);
  ...
  it('falls back to uniform when nothing in the pool can attack', () => {
    expect(getTypeThreatWeights([], TYPES)).toBe(UNIFORM_TYPE_THREAT);
  });
  ...
  it('prices a weakness by what the pool can bring, not by what it is', () => {
    ...
    const shares = measureTypeThreat(withoutFightingTypes, TYPES);
    const weights = getTypeThreatWeights(withoutFightingTypes, TYPES);
